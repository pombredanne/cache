...
func NewCatalog() *Catalog {
	...
	c, _ := serde.From[*Catalog](response.Body, serde.JSON)
	return c
}
...
func (c *Catalog) Each(visitor core.Visitor[*core.Dependency]) {
	...
	for _, c := range c.Items {
		...
		cpd, err := serde.From[CatalogPageData](response.Body, serde.JSON)
		if err != nil {
			fmt.Printf("%v\n", err)
			continue
		}
		...
		for _, item := range cpd.Items {
			urls = append(urls, fmt.Sprintf("%s%s/index.json", registrationBaseUrl, strings.ToLower(item.Name)))
		}
	}
	...
	for _, url := range urls {
		...
		data, err := serde.From[PackageIndexData](response.Body, serde.JSON)
		if err != nil {
			fmt.Printf("%v\n", err)
			continue
		}
